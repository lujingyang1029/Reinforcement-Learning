...
class BaseAgent:
	...
	def __init__(self, alpha, epsilon, discount, action_space, state_space):
		...
		self.vvalues = np.zeros(state_space, np.float32)
	...
	def update(self, state, action, reward, next_state, next_state_possible_actions, done):
		...
		# Store the best state so far for optimization
		vval_old = self.get_value(next_state, next_state_possible_actions)
		if self.vvalue_max<= vval_old:
			self.vvalue_max = vval_old
			self.state_selected = next_state
